...
// TODO Check each of these and document why it's needed (if it really is)
const ALWAYS_SORT_FIRST: [i64; 4] = [
    171, 203, // TeamFormed events must be before the corresponding following events
    244,
...
impl Ord for SpecialSortingEventType {
    fn cmp(&self, other: &Self) -> Ordering {
        match (
            ALWAYS_SORT_FIRST.contains(&self.0),
            ALWAYS_SORT_FIRST.contains(&other.0),
        ) {
...
fn main() {
    ...
    let feed_era_start = chrono::DateTime::parse_from_rfc3339("2021-03-01T05:00:00.000Z")
        .unwrap()
        .with_timezone(&Utc);
    ...
    let mut vec: Vec<_> = reader
        .lines()
        .map(parse)
        .filter(move |(_, has_parent, _, _, created, _)| created >= &feed_era_start && !*has_parent)
        .collect();
    println!("Sorting by season...");
    vec.sort_by_key(|(_, _, season, sim, _, _)| (sim.clone(), *season));
    let groups = vec
        .into_iter()
        .group_by(|(_, _, season, sim, _, _)| (sim.clone(), *season));

    println!("Splitting seasons...");
    for ((sim, season), group) in &groups {
        println!("Processing sim {sim} season {season}...");
        let mut vec: Vec<_> = group
            .map(|(s, _, _, _, created, event_type)| (s, created, event_type))
            .collect();

        println!("Sorting sim {sim} season {season}...");
        vec.sort_by_key(|(_, created, event_type)| {
            (*created, SpecialSortingEventType(*event_type))
        });
        println!("Writing sim {sim} season {season}...");
        std::fs::write(
            format!("feed_dump_filtered/sim-{sim}-season-{season}.ndjson"),
