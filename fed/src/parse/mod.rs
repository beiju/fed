...
pub fn parse_next_event(
    ...
    let data = match event.event_type {
        ...
        EventType::FlyOut => {
            ...
            let mut scores = event.parse_scores_without_summary(" tags up and scores!")?;
            let batter_item_damage = event.parse_item_damage(batter_name)?;
            ...
            match event.next_parse(parse_ground_out)? {
                ParsedGroundOut::Simple {
                    ...
                    let (scoring_players, attractions) =
                        event.parse_scoring_players(" advances on the sacrifice.")?;
                    let batter_item_damage = event.parse_item_damage(batter_name)?;
                ...
                ParsedGroundOut::FieldersChoice {
                    ...
                    let attractions = event.parse_attractions()?;
                ...
                ParsedGroundOut::DoublePlay { batter_name } => {
                    let mut scores = event.parse_scores_without_summary(" scores!")?;
                    let stopped_inhabiting = event.parse_stopped_inhabiting(None)?;
                    let cooled_off = event.parse_cooled_off(batter_name)?;
                    let flood_balloon_popped = event.parse_flood_balloon_popped();
                    scores.score_summary = event.parse_score_summary()?;
            ...
            match event.next_parse(parse_trade(event.is_post_semicentennial()))? {
                ...
                ParsedTrade::Traded {
                    ...
                    let (game, mut trader_event, mut victim_event) = if is_trade_for_nothing {
                        ...
                        let trader_event = event.next_child(EventType::PlayerGainedItem)?;

                        // In this case game is after all the other children, as usual
                        let game = event.game(unscatter, attractor_secret_base)?;

                        (game, trader_event, victim_event)
                    } else {
                        // For this event specifically, ride-along trades' children appear before
                        // the other children, so game has to be constructed first.
                        let game = event.game(unscatter, attractor_secret_base)?;

                        let trader_event = event.next_child(EventType::ItemTraded)?;
                        let victim_event = event.next_child(EventType::ItemTraded)?;

                        (game, trader_event, victim_event)
                    };

                    let build_trade_for_something = || {
