...
pub fn events(start: &'static str) -> impl Stream<Item = EventuallyEvent> {
    eventually_pages(start, BUFFER_PAGES)
        ...
        .scan(HashSet::new(), |seen_ids, mut event| {
            ...
            let id_order: HashMap<_, _> = event
                .metadata
                .sibling_ids
                .iter()
                .flatten()
            ...
            event
                .metadata
                .siblings
                .sort_by_key(|event| id_order.get(&event.id).unwrap());
...
fn sort_children(children: &mut Vec<EventuallyEvent>) {
    ...
    for (a, b) in children.iter().tuple_windows() {
        assert!(
            a.metadata.sub_play != b.metadata.sub_play,
            "All subPlays should be unique"
        );
    }
...
fn eventually_pages(
    start: &'static str,
    buffer_pages: usize,
) -> impl Stream<Item = Vec<EventuallyEvent>> {
    ...
    stream::unfold(1, move |page| {
        ...
        async move { Some(((page, cache, client), page + 1)) }
    })
    ...
    .map(move |(page, cache, client)| async move {
        let request = client
            .get("https://api.sibr.dev/eventually/v2/events")
        ...
        let response = match cache.get(&cache_key).unwrap() {
            ...
            None => {
                ...
                cache
                    .insert(&cache_key, pot::to_vec(&response).unwrap())
                    .unwrap();
